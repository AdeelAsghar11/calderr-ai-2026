...
if __name__ == "__main__":
    ...
    r1 = run_case(graph, "sample_docs/short_note.txt", "NORMAL branch (under threshold)")
    ...
    r2 = run_case(graph, "sample_docs/long_report.txt", "OVERSIZED branch (split first)")
    assert r2["is_valid"] is True
    assert r2["is_oversized"] is True
    assert len(r2["parts"]) > 1
    assert r2["num_embedded"] == len(r2["chunks"])

    r3 = run_case(graph, "sample_docs/empty_file.txt", "INVALID branch (whitespace only)")
    assert r3["is_valid"] is False

    r4 = run_case(graph, "sample_docs/does_not_exist.txt", "INVALID branch (missing file)")
