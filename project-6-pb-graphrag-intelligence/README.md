...
```text
project-6-pb-graphrag-intelligence/
├── README.md                 # System architecture, setup commands, & evaluation status
├── BLOG.md                   # Drafted blog post source content for external publishing
...
├── evaluator.py              # 90-sample evaluation runner (stub & real ChatGroq)
...
├── models.py                 # Core domain data models (QuestionRecord, MethodResult)
├── report_generator.py       # HTML report exporter (evaluation_report.html)
├── router.py                 # Pre-retrieval query classifier
├── statistical_test.py       # Paired t-test (scipy.stats.ttest_rel) calculation
...
Run the 6 automated test proofs (scale correctness, sibling expansion, statistics, pipeline wiring, ambiguity check, HTML report):

```bash
uv run python project-6-pb-graphrag-intelligence/smoke_test.py
```
...
```bash
uv run python project-6-pb-graphrag-intelligence/cli.py run-study
```
...
```bash
uv run uvicorn project-6-pb-graphrag-intelligence.api:app --reload --port 8000
```
...
```bash
uv run streamlit run project-6-pb-graphrag-intelligence/dashboard.py
```
...
## 📊 Evaluation Results & Honesty Disclosure
