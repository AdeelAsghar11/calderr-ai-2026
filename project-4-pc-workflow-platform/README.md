...
```
project-4-pc-workflow-platform/
├── README.md
├── BLOG.md
├── Dockerfile
├── docker-compose.yml
...
```bash
uv run python -m pytest project-4-pc-workflow-platform/tests/
```

### 2. Launch FastAPI Server
```bash
uv run uvicorn project-4-pc-workflow-platform.src.api:app --reload --port 8000
```
- Interactive API Docs: `http://localhost:8000/docs`

### 3. Launch Streamlit Dashboard
```bash
uv run streamlit run project-4-pc-workflow-platform/src/dashboard.py
```
- Access Dashboard: `http://localhost:8501`

### 4. Run via Docker Compose
```bash
docker-compose -f project-4-pc-workflow-platform/docker-compose.yml up --build
```
